...
If I was given more time to work on the web app I would work more on the frontend to make less plain and more attractive to users. I would also add the ability for users to edit or destroy their accounts. This would allow users to change/recover passwords, change usernames, etc. and also get rid of all secrets stored in the database if they no longer wish to store them. Another feature I could add if given more time is a feature to share either all secrets or individual secrets with another user as sometimes we wish to share secrets with friends. 
   +let users share all or individual secrets
   +change database for deployment
